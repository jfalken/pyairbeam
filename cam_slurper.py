...
class CamSlurper:
    ...
    def _rget(self, path):
        ...
        if r.status_code == 200:
            ...
            logging.error('Error getting %s: -> %s' % (url, str(sys.exc_info())))
    ...
    def get_duration(self):
        ...
        logging.debug('Raw Duration time: %s' % time)
        try:
            hours, mins, secs = time.split(':')
            duration = int(secs) + (int(mins) * 60) + (int(hours) * 3600)
            logging.debug('Current Duration: %d' % duration)
            return duration
        except:
            return 0
    ...
    def list_recordings(self):
        ...
        logging.debug('Current files: %s' % str(recordings))
        return recordings

    def download_recording(self, file_name):
        url = self.hostname + '/recording/' + file_name
        path = self.storage_path + str(time.time())[:10] + '_' + file_name
        with open(path, 'wb') as handle:
            response = requests.get(url, stream=True)

            if not response.ok:
                logging.error('Could not download/write file -> %s' % sys.exc_info())
                return 'not ok'
            ...
            for block in response.iter_content(1024):
                ...
                handle.write(block)
        logging.info('Downloaded file %s; saved to %s' % (file_name, path))
        return 'ok'

    def delete_recording(self, file_name):
        ''' Deletes a recording '''
        r = self._rget('delete/' + str(file_name))
        logging.info('Device [%s]: "%s" deleted.' % (self.name, str(file_name)))
    ...
    def remove_oldest_file(self):
        ...
        os.remove(delfile)
        logging.info('Deleted old file %s' % delfile)

def main(args, config):
    logging = setup_logging(config=config, args=args)

    cname      = config['camera']['name']
    hostname   = config['camera']['hostname']
    store_path = config['camera']['store_path']
